...
export class Room {
    ...
    getPlayers(...statuses) {
        return this.playerIds.reduce((playerList, id) => {
    ...
    startGame(maxRounds = 0) {
        ...
        const isFinalStage = this.stage === GameStage.Results &&
            this.maxRounds &&
        ...
        switch (this.stage) {
            case GameStage.Question:
                if (typeof payload !== 'string' ||
                    ...
                    return;
                const playerAnswer = this.answers.find((answer) => answer.playerId === playerId);
                if (playerAnswer) {
                ...
                break;
            case GameStage.Vote:
                if (typeof payload !== 'number' || payload < 0 || payload > 2)
                    return;
